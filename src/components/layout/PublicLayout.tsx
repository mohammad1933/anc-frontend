      ...
      <header className="public-header">
        <div className="public-nav">
          ...
          <div className="public-actions">
            {user ? <>{user.role === "admin" && <Link className="public-dashboard-link" to="/dashboard">DASHBOARD</Link>}<div className="account-menu"><button type="button" aria-label="Open account menu" onClick={()=>setAccountOpen(!accountOpen)}>{user.name.split(" ").map(part=>part[0]).slice(0,2).join("")}</button>{accountOpen&&<nav className="account-panel"><strong>{user.name}<small>{user.email}</small></strong>{user.role === "admin" ? <Link to="/dashboard" onClick={()=>setAccountOpen(false)}>Dashboard</Link> : <><Link to="/projects" onClick={()=>setAccountOpen(false)}>Projects</Link><Link to="/favorites" onClick={()=>setAccountOpen(false)}>Favorites</Link></>}<button type="button" onClick={logout}>Log Out</button></nav>}</div></> : <><Link className="public-auth-link" to="/login">SIGN IN</Link><Link className="public-admin-login" to="/admin/login">ADMIN LOGIN</Link><Link className="public-auth-link" to="/register">REGISTER</Link></>}
          </div>
          <button className="public-menu" type="button" aria-label="Toggle menu" onClick={() => setOpen(!open)}>{open ? "×" : "☰"}</button>
        </div>
        {open && <nav className="public-mobile">{links.map(([label, to]) => <Link onClick={() => setOpen(false)} to={to} key={to}>{label}</Link>)}<Link onClick={() => setOpen(false)} to="/request-sample">REQUEST SAMPLE</Link>{user ? <>{user.role === "admin" ? <Link onClick={() => setOpen(false)} to="/dashboard">DASHBOARD</Link> : <><Link onClick={() => setOpen(false)} to="/projects">PROJECTS</Link><Link onClick={() => setOpen(false)} to="/favorites">FAVORITES</Link></>}<button onClick={() => { logout(); setOpen(false); }}>LOG OUT</button></> : <><Link onClick={() => setOpen(false)} to="/login">SIGN IN</Link><Link className="public-admin-login" onClick={() => setOpen(false)} to="/admin/login">ADMIN LOGIN</Link><Link onClick={() => setOpen(false)} to="/register">REGISTER</Link></>}</nav>}
      </header>
